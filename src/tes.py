...
def voice_change_pipeline(song_input, voice_model, pitch_change, is_webui=0, pitch_change_all=0):
    try:
        ...
        display_progress('[~] Starting Voice Change Pipeline...', 0, is_webui)

        with open(os.path.join(mdxnet_models_dir, 'model_data.json')) as infile:
            mdx_model_params = json.load(infile)

        # Determine the input type (local file or YouTube URL) and process accordingly.
        ...
        if not os.path.exists(song_dir):
            raise_exception(f'Song data for {song_id} is missing. Run the full pipeline first.', is_webui)

        orig_song_path, main_vocals_dereverb_path, backup_vocals_path = get_audio_paths(song_dir)
        ...
        if not os.path.exists(ai_vocals_path):
            ...
            voice_change(voice_model, main_vocals_dereverb_path, ai_vocals_path, pitch_change, 'rmvpe', 0.5, 3, 0.25, 0.33, 128, is_webui)

        display_progress('[~] Voice change completed.', 1, is_webui)
