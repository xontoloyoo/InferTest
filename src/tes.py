...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress=None):
    ...
    #display_progress('[~] Separating Vocals...', 0.1, is_webui, progress)
    #vocals_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'UVR-MDX-NET-Voc_FT.onnx'), orig_song_path, denoise=True, keep_orig=keep_orig)
   
    #display_progress('[~] Applying DeReverb to Vocals...', 0.3, is_webui, progress)
    #vocals_dereverb_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'Reverb_HQ_By_FoxJoy.onnx'), vocals_path, invert_suffix='DeReverb', exclude_main=True, denoise=True)

    display_progress('[~] Applying DeReverb to Vocals...', 0.3, is_webui, progress)
    vocals_dereverb_path = run_mdx(mdx_model_params, song_output_dir, os.path.join(mdxnet_models_dir, 'Reverb_HQ_By_FoxJoy.onnx'), orig_song_path, invert_suffix='DeReverb', exclude_main=True, denoise=True)

    
    #return orig_song_path, vocals_path, instrumentals_path, vocals_dereverb_path
    return orig_song_path, vocals_dereverb_path


def voice_change(voice_model, vocals_dereverb_path, output_path, pitch_change, f0_method, index_rate, filter_radius, rms_mix_rate, protect, crepe_hop_length, is_webui):
    ...
    # convert main vocals
    rvc_infer(rvc_index_path, index_rate, vocals_dereverb_path, output_path, pitch_change, f0_method, cpt, version, net_g, filter_radius, tgt_sr, rms_mix_rate, protect, crepe_hop_length, vc, hubert_model)
...
def combine_audio(audio_paths, output_path, main_gain, backup_gain, inst_gain, output_format):
    ...
    vocal_audio.overlay(instrumental_audio).export(output_path, format=output_format)
...
def song_cover_pipeline(song_input, voice_model, pitch_change, keep_files,
    ...
    try:
        ...
        if not os.path.exists(song_dir):
            ...
            orig_song_path, vocals_dereverb_path = preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress)
        ...
        combine_audio([ai_vocals_mixed_path, Instrumentals_path], ai_cover_path, output_format)
