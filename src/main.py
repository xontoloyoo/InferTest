...
def get_youtube_video_id(url, ignore_playlist=True):
    # (Fungsi get_youtube_video_id telah dibiarkan karena terkait dengan input YouTube)

def yt_download(link):
    # (Fungsi yt_download telah dibiarkan karena terkait dengan input YouTube)

def raise_exception(error_msg, is_webui):
    # (Fungsi raise_exception telah dibiarkan karena digunakan untuk menangani kesalahan)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-k', '--keep-files', action='store_true', help='Whether to keep all intermediate audio files generated in the song_output/id directory, e.g. Isolated Vocals/Instrumentals')
