...
def get_audio_paths(song_dir):
    ...
    for file in os.listdir(song_dir):
        if file.endswith('_Instrumental.wav'):
            opt_ins_root = os.path.join(song_dir, file)
            dir_wav_input = instrumentals_path.replace('_Instrumental', '')

        elif file.endswith('_Vocals_Main_DeReverb.wav'):
            opt_vocal_root = os.path.join(song_dir, file)

        #elif file.endswith('_Vocals_Backup.wav'):
        #    backup_vocals_path = os.path.join(song_dir, file)

    #return orig_song_path, instrumentals_path, main_vocals_dereverb_path, backup_vocals_path
    return dir_wav_input, opt_ins_root, opt_vocal_root
...
def preprocess_song(song_input, mdx_model_params, song_id, is_webui, input_type, progress=None):
    ...
    dir_wav_input = convert_to_stereo(dir_wav_input)

    display_progress('[~] Separating Vocals from Instrumental...', 0.1, is_webui, progress)
    save_root_vocal, save_root_ins = uvr(model_name, song_output_dir, os.path.join(mdxnet_models_dir, 'HP5_only_main_vocal.pth'), dir_wav_input, denoise=True, keep_orig=keep_orig)
...
def song_cover_pipeline(song_input, keep_files, is_webui=0, output_format='mp3',
        ...
        with open(os.path.join(mdxnet_models_dir, 'name_params.json')) as infile:
        ...
        if not os.path.exists(song_dir):
            ...
            dir_wav_input, opt_ins_root, opt_vocal_root = preprocess_song(song_input, model_name, song_id, is_webui, input_type, progress)
        else:
            ...
            if any(path is None for path in paths) or keep_files:
                ...
                dir_wav_input, opt_ins_root, opt_vocal_root = paths
        ...
        if not keep_files:
            ...
            intermediate_files = [save_root_vocal, save_root_ins]
